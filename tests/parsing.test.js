...
describe('Data Parsers', () => {
  ...
  describe('parseJson', () => {
    ...
    test('should parse a valid JSON object', () => {
      ...
      const expected = [{ type: 'checking', name: 'My Checking', balance: 5000 }];
      expect(parseJson(json)).toEqual(expected);
    });
    ...
    test('should handle string balances', () => {
        ...
        const expected = [{ type: 'savings', name: 'My Savings', balance: 12000 }];
        expect(parseJson(json)).toEqual(expected);
    });
    ...
    test('should throw an error if no valid accounts are found', () => {
      ...
      expect(() => parseJson(json)).toThrow('No valid accounts found in JSON response');
    });
  });
...
"My Checking",5000`;
      const rows = parseCsv(csv);
      const expected = [{ type: undefined, name: 'My Checking', balance: 5000 }];
      expect(parseArray(rows)).toEqual(expected);
    });
...
checking,5000`;
      const rows = parseCsv(csv);
      expect(() => parseArray(rows)).toThrow('Sheet is missing a "name" column header');
    });
...
checking,"My Checking"`;
      const rows = parseCsv(csv);
      expect(() => parseArray(rows)).toThrow('Sheet is missing a "balance" column header');
    });

    test('should throw an error for no data rows', () => {
      const csv = `type,name,balance`;
      const rows = parseCsv(csv);
      expect(() => parseArray(rows)).toThrow('Sheet has no data rows (need header + at least one data row)');
    });
  });
});
